...
func TestGetSchema(t *testing.T) {
	...
	if nil != err {
		t.Error("Could not marshall CPU Schema: ", err)
	}
	log.Println("CPU Schema Size: ", len(bytes))
	data = &schema.Schema{}
	err = proto.Unmarshal(bytes, data)
	if nil != err {
		t.Error("Could not unmarshall CPU Schema: ", err)
	}
...
func TestGetData(t *testing.T) {
	...
	if nil != err {
		t.Error("Could not get CPU Data: ", err)
	}
	bytes, err := proto.Marshal(data)
	if nil != err {
		t.Error("Could not marshall CPU Data: ", err)
	}
	log.Println("CPU Data Size: ", len(bytes))
	data = &dataset.DataSet{}
	err = proto.Unmarshal(bytes, data)
	if nil != err {
		t.Error("Could not unmarshall CPU Data: ", err)
	}
	//	log.Println(Collectors.ToJSON(data))
}
